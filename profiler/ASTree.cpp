...
AST::AST(const AST& actual) {
    ...
    for (std::list<AST*>::const_iterator it = actual.child.begin(); it != actual.child.end(); ++it) {
        AST* originalChild = *it;
        AST* cloneChild = new AST(*originalChild);
        child.push_back(cloneChild);
    }
}
...
void AST::insertMainHeader(const std::vector<std::string>& profileName,
    ...
    for (std::list<AST*>::iterator it = child.begin(); it != child.end(); ++it) {
        if((*it)->tag == "function") {
            index = it;
            AST* include = new AST(category, "cpp:include");
            include->child.push_back(new AST(token, "#include"));
            include->child.push_back(new AST(whitespace, " "));
            include->child.push_back(new AST(token, "\"profile.hpp\""));
            include->child.push_back(new AST(whitespace, "\n"));
            child.insert(it, include);
            break;
        }
    }
    ...
    for (int i = 0; i < size; ++i) {
        AST* profiles = new AST(category, "expr_stmt");
        profiles->child.push_back(new AST(token, "profile"));
        profiles->child.push_back(new AST(whitespace, " "));
        profiles->child.push_back(new AST(token, profileName[i]));
        profiles->child.push_back(new AST(token, "("));
        profiles->child.push_back(new AST(token, "\"" + fileName[i] + "\""));
        profiles->child.push_back(new AST(token, ")"));
        profiles->child.push_back(new AST(token, ";"));
        profiles->child.push_back(new AST(whitespace, "\n"));

        child.insert(index, profiles);
        ++index;
    }
}
...
void AST::insertFileHeader(const std::string& profileName) {
    ...
    std::list<AST*>::iterator index = child.begin();
    while(index != child.end() && (*index)->tag != "function") {
        ++index;
    }
    ...
    include->child.push_back(new AST(token, "\"profile.hpp\""));
    include->child.push_back(new AST(whitespace, "\n"));
    child.insert(index, include);
    ++index;

    AST* temp = new AST(category, "decl_stmt");
    temp->child.push_back(new AST(token, "extern"));
    temp->child.push_back(new AST(whitespace, " "));
    temp->child.push_back(new AST(token, "profile"));
    temp->child.push_back(new AST(whitespace, " "));
    temp->child.push_back(new AST(token, profileName));
    temp->child.push_back(new AST(token, ";"));
    temp->child.push_back(new AST(whitespace, "\n"));
    child.insert(index, temp);
}
...
void AST::insertMainReport(const std::vector<std::string>& profileName) {
    ...
    std::list<AST*>::iterator funcIt = child.begin();
    while (funcIt != child.end()) {
        if ((*funcIt)->tag == "function") {
            AST* main = (*funcIt)->getChild("name");
            if (main && main->getName() == "main") break;
        }
        ++funcIt;
    }
    if (funcIt == child.end()) return; // no main

    AST* funcNode = *funcIt;

    std::list<AST*>::iterator blockIt = funcNode->child.begin();
    while (blockIt != funcNode->child.end()) {
        if ((*blockIt)->tag == "block") break;
        ++blockIt;
    }
    if (blockIt == funcNode->child.end()) return; // no block

    AST* blockNode = *blockIt;

    std::list<AST*>::iterator bcIt = blockNode->child.end();
    --bcIt;
    while(bcIt != blockNode->child.begin()) {
        if ((*bcIt)->tag == "block_content") break;
        --bcIt;
    }
    if (bcIt == blockNode->child.begin()) return; // no block_content

    AST* bcNode = *bcIt;
    
    std::list<AST*>::iterator lastRetIt = bcNode->child.end();
    std::list<AST*>::iterator it = bcNode->child.end();
    --it;
    while (it != bcNode->child.begin()) {
        if ((*it)->tag == "return") {
            lastRetIt = it;
            break;
        }
    ...
    if (lastRetIt == bcNode->child.begin()) return; // no return at the end of main

    int size = profileName.size();
    for (int i = 0; i < size; ++i) {
        AST* report = new AST(category, "expr_stmt");
        AST* name1 = new AST(category, "name");
        name1->child.push_back(new AST(token, "std"));
        name1->child.push_back(new AST(token, "::"));
        name1->child.push_back(new AST(token, "cout"));
        report->child.push_back(name1);

        report->child.push_back(new AST(whitespace, " "));
        report->child.push_back(new AST(token, "<<"));
        report->child.push_back(new AST(whitespace, " "));
        report->child.push_back(new AST(token, profileName[i]));
        report->child.push_back(new AST(whitespace, " "));
        report->child.push_back(new AST(token, "<<"));
        report->child.push_back(new AST(whitespace, " "));

        AST* name2 = new AST(category, "name");
        name2->child.push_back(new AST(token, "std"));
        name2->child.push_back(new AST(token, "::"));
        name2->child.push_back(new AST(token, "endl"));
        report->child.push_back(name2);

        report->child.push_back(new AST(token, ";"));
        report->child.push_back(new AST(whitespace, "\n"));

        bcNode->child.insert(lastRetIt, report);
        ++lastRetIt;
    }
}
...
void AST::insertFunctionCount(const std::string& profileName) {
    ...
    for (std::list<AST*>::iterator fcd = child.begin(); fcd != child.end(); ++fcd) {
        if ((*fcd)->tag == "function" || (*fcd)->tag == "constructor" || (*fcd)->tag == "destructor") {
            AST* nameNode = (*fcd)->getChild("name");
            std::string fName = nameNode->getName();

            std::list<AST*>::iterator block = (*fcd)->child.begin();
            while (block != (*fcd)->child.end()) {
                if ((*block)->tag == "block") break;
                ++block;
            }
            if (block == (*fcd)->child.end()) {
            std::cerr << "Error: no block" << std::endl; //no block
            continue;
            }

            std::list<AST*>::iterator bc = (*block)->child.begin();
            ++bc;
            while (bc != (*block)->child.end()) {
                if ((*bc)->tag == "block_content") break; 
                ++bc;
            }
            if (bc == (*block)->child.end()) {
            std::cerr << "Error: no block_content" << std::endl; //no block_content
            continue; 
            }

            AST* countStmt = new AST(category, "expr_stmt");
            countStmt->child.push_back(new AST(token, profileName));
            countStmt->child.push_back(new AST(token, "."));
            countStmt->child.push_back(new AST(token, "count"));
            countStmt->child.push_back(new AST(token, "("));
            countStmt->child.push_back(new AST(token, "__LINE__"));
            countStmt->child.push_back(new AST(token, ","));
            countStmt->child.push_back(new AST(whitespace, " "));
            countStmt->child.push_back(new AST(token, "\"" + fName + "\""));
            countStmt->child.push_back(new AST(token, ")"));
            countStmt->child.push_back(new AST(token, ";"));
            countStmt->child.push_back(new AST(whitespace, "\n"));

            (*bc)->child.insert((*bc)->child.begin(), countStmt);
        }
    }
}
...
void AST::insertLineCount(const std::string& profileName) {
    ...
    for(std::list<AST*>::iterator it = child.begin(); it != child.end(); ++it) {
        if ((*it)->nodeType == category) {
            std::list<AST*>::iterator block = (*it)->child.begin();
            while (block != (*it)->child.end()) {
                if ((*block)->tag == "block") break;
                ++block;
            }
            if (block == (*it)->child.end()) {
                continue;
            }

            AST* bc = nullptr;
            bc = (*block)->getChild("block_content");
            if (!bc) { 
                continue;
            }
            std::list<AST*>::iterator i = bc->child.begin();
            while (i != bc->child.end()) {
                if ((*i)->tag == "expr_stmt") {
                    if ((*i)->child.front()->nodeType == token && (*i)->child.front()->text == profileName) {
                        ++i;
                        continue;
                    }
                    AST* countStmt = new AST(category, "expr_stmt");
                    ...
                    countStmt->child.push_back(new AST(token, ";"));
                    countStmt->child.push_back(new AST(whitespace, "\n"));
                    std::list<AST*>::iterator expr = std::next(i);
                    bc->child.insert(expr, countStmt);
                    i = std::next(expr);
                    continue;
                } else if (!(isStopTag((*i)->tag))){
                    (*i)->insertLineCount(profileName);
                }
                ++i;
            }
        }
